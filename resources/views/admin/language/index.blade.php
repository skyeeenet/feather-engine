...
@section('content')
    ...
    <div class="roboto16">
      ...
      <div class="mt-5 d-flex align-items-center justify-content-center justify-content-sm-start flex-wrap">
        ...
        <span class="users_text mx-3">Языки</span>
        <a href="{{route('admin.languages.create')}}" class="btn btn-success mx-2 my-1"><i
    ...
    <div class="bg-white shadow d-block mb-5 roboto14">
      ...
      <div class="table-responsive-xl">
        <table class="table">
          ...
          <tr class="border-bottom">
            ...
            <th scope="col" class="pt-2 pl-2 pb-2 pr-0 text-center"><a class="sort" href="#">Идентификатор</a></th>
            <th scope="col" class="pt-2 pl-2 pb-2 pr-0 text-center"><a class="sort" href="#">Ключевое значение</a></th>
          ...
          @foreach($languages as $item)
            <tr class="border-bottom">
              ...
              <td scope="col" class="p-2" align="center">{{$item->id ?? ''}}</td>
              <td scope="col" class="p-2" align="center">{{$item->key ?? ''}}</td>
              <td scope="col" class="p-2">
                <div class=" d-flex flex-column align-items-center">
                  <form action="{{route('admin.languages.destroy', $item->id)}}" method="post">
                    @csrf
                  ...
                  <a href="{{route('admin.languages.edit', $item->id)}}" class="btn btn-primary my-1">
      ...
      <div class="mt-4 d-flex justify-content-sm-between justify-content-center pb-5 mx-2 flex-wrap">
        <div class="d-flex align-items-center mb-2">
          <span>Показано от 1 до {{$languages->count()}} из {{$languages->total()}} записей</span>
        </div>
        {{$languages->links()}}
      </div>
    </div>
